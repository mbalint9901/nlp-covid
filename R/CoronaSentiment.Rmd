...
update_geom_defaults("col", list(color = "black", fill = "cyan4", size = 1.4))
...
```{r}
topic_models <- list.files(str_c(WD, "/data/topic_models"), 
           pattern = "topic_model", 
           full.names = TRUE) %>% 
  map(function(x) {load(x); return (mod)}) %>% 
...
```{r fig.cap="Correlation matrix of topics' gamma"}
dat_topics %>% 
